...
function createInputFields() {
    ...
    inputFieldHTML += '<tr><th>Process</th><th>Arrival Time</th><th>Burst Time</th><th>Priority</th></tr>';
    for (let i = 0; i < numProcesses; i++) {
        inputFieldHTML += `
            <tr>
                ...
                <td><input type="number" id="priority${i}" class="priority-input" min="1"></td>
            </tr>
        `;
    }
...
function calculateTimes() {
    ...
    if (numProcesses) {
        ...
        for (let i = 0; i < numProcesses; i++) {
            ...
            if (arrivalTime.trim() === '' || burstTime.trim() === '' || priority.trim() === '') {
        ...
        if (allInputsFilled) {
            ...
            for (let i = 0; i < numProcesses; i++) {
                ...
                let burstTime = document.getElementById(`burstTime${i}`).value;
                let priority = document.getElementById(`priority${i}`).value;
                processes.push({arrivalTime: parseInt(arrivalTime), burstTime: parseInt(burstTime), priority: parseInt(priority), completionTime: 0, turnAroundTime: 0, waitingTime: 0});
            }
...
function displayTable(avgWaitingTime, avgTurnAroundTime) {
    ...
    tableHTML += '<tr><th>Process</th><th>Arrival Time</th><th>Burst Time</th><th>Priority</th><th>Completion Time</th><th>Turn Around Time</th><th>Waiting Time</th></tr>';
    for (let i = 0; i < processes.length; i++) {
        tableHTML += `<tr><td>Process ${i+1}</td><td>${processes[i].arrivalTime}</td><td>${processes[i].burstTime}</td><td>${processes[i].priority}</td><td>${processes[i].completionTime}</td><td>${processes[i].turnAroundTime}</td><td>${processes[i].waitingTime}</td></tr>`;
    }
